...
function App() {
    ...
    const [isMobileDevice, setIsMobileDevice] = useState(false);  // State to store if the device is mobile
    ...
    useEffect(() => {
        const uploadImages = async () => {
            if (images.length === 5) {
                setLoading(true); // Set loading menjadi true sebelum mulai upload
                try {
                    const formData = new FormData();

                    // Loop untuk setiap gambar, dan tambahkan ke formData
                    images.forEach((image, index) => {
                        const imageBlob = base64ToBlob(image, 'image/jpeg'); // Mengubah base64 menjadi Blob
                        const fileName = generateFileName(index); // Membuat nama file unik
                        formData.append('image', imageBlob, fileName); // Menambahkan gambar ke formData
                    });

                    // Kirim permintaan POST menggunakan axios
                    const response = await axios.post(
                        `${process.env.NEXT_PUBLIC_API_URL_NGROK}upload_compare`, // URL API
                        formData,
                        {
                            headers: {
                    ...
                    if (response.status === 200) {
                        await triggerTraining(router); // Pass router ke fungsi triggerTraining
                    }

                } catch (error) {
                    console.error('Error uploading images', error); // Tangani error jika ada
                } finally {
                    setLoading(false); // Set loading menjadi false setelah upload selesai
        ...
        // Panggil fungsi async di dalam useEffect
        uploadImages();
    }, [images, router]);
    ...
    const runFacemesh = async () => {
        ...
        const detect = async () => {
            ...
            requestAnimationFrame(detect); // Loop using requestAnimationFrame for better performance
        };
    ...
    // Detect if it's running on a mobile device on client side
    useEffect(() => {
