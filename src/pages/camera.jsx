...
const CameraPage = () => {
  ...
  const getDevices = async () => {
    try {
      ...
      handleError(
        "Tidak dapat mengakses perangkat kamera. Pastikan perangkat kamera terhubung dengan benar."
      );
    }
  };
  ...
  const startVideo = async (deviceId) => {
    try {
      ...
      handleError(
        `Tidak dapat mengakses kamera. Pastikan kamera diaktifkan dan izinkan akses kamera.${err}`
      );
    }
  };
        ...
        axios
          ...
          .then((response) => {
            ...
            localStorage.setItem(
              "capturedPhotos",
              JSON.stringify([...existingPhotos, photoURL])
            );
            redirectToResult();
          })
          .catch((error) => {
  ...
  const redirectToResult = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        // setTimeout(() => router.push("/photo-result"), 1000);
      })
      .catch((error) => {
        setLoading(false);
        Swal.fire({
  ...
  const fetchMatchData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}scanner/get-match`
      );
      ...
      <div className="relative flex flex-col items-center justify-center">
        {error ? (
          ...
          <>
            <video
              ref={videoRef}
              className="border border-gray-400 rounded-lg shadow-lg w-full max-w-md h-auto"
              autoPlay
            ></video>
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div role="status" className="flex flex-col items-center w-[">
            <svg
              aria-hidden="true"
              className="w-52 h-50 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
              viewBox="0 0 100 101"
              fill="none"
